...
bpy.ops.wm.save_mainfile(filepath=("E:\\AWS\\My Project\\AWS_Blender_programs\\Draw_in_Blender_VPC_With_VPN_Connection.blend"))
    ...
    for sub, clas in subnet_type.items():
        for subnet, coord in obj_offset.items():
            if (sub in subnet and "Public" in clas  or "public" in clas):
                new_co_ords = draw_resources_in_public_subnet(sub, coord)
                blender_methods.draw_public_sub(coord)
            elif (sub in subnet and "Private" in clas  or "private" in clas):
                new_co_ords = draw_resources_in_private_subnet(sub, coord)
...
def draw_resources_in_public_subnet(subnet_name, coordinates):                 # Called by draw_subnets()
...
def draw_resources_in_private_subnet(subnet_name, coordinates):                 # Called by draw_subnets()
    for subnet, resources in res_in_subnets.items():
        if(subnet == subnet_name):
            local_coord = coordinates
            if (resources[0] != '' and "MultiAZ" in resources[0]):
...
def draw_public_vpc_stack():
    ...
    for vpc in blend_data.vpc_name:
        for key, value in(vpc_pub_stack.items()):
            ...
            for elb in reversed_stack[1]:                   # Remember: ELB needs subnets or CIDR or Instances which are publicly accessible.
...
print (coordinate_tracker)      # Before draw_subnets, its contents should be empty.
draw_subnets()
print (coordinate_tracker)      # This keeps track of Subnets.


private_vpc_tracker = draw_private_vpc_stack()
print (private_vpc_tracker)
